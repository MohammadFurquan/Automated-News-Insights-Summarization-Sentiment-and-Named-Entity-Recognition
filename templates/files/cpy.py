...
def scrape_news_route():
    ...
    # Save to Database
    save_to_db(url, summary, sentiment, ner, related_topic)
...
init_db()  # Call this function at startup

# Function to save search history
def save_to_db(url, summary, sentiment, ner, related_topic):
    conn = sqlite3.connect("news_history.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO history (url, summary, sentiment, ner, related_topic) VALUES (?, ?, ?, ?, ?)",
                   (url, summary, sentiment, ner, related_topic))
    conn.commit()
    conn.close()
...
def fetch_history():
    conn = sqlite3.connect("news_history.db")
    cursor = conn.cursor()
    cursor.execute("SELECT url, summary, sentiment, ner, related_topic, timestamp FROM history ORDER BY timestamp DESC LIMIT 10")
